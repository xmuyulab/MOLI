...
save_results_to = os.path.join(root_dir,'results/SoftClassifierTripNetv16/Gemcitabine/')
if not os.path.exists(save_results_to):
    os.makedirs(save_results_to)
max_iter = 50
torch.manual_seed(42)

GDSCE = pd.read_csv(os.path.join(root_dir,'data/expr_homogenized/')+"GDSC_exprs.Gemcitabine.eb_with.PDX_exprs.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ",")
GDSCE = pd.DataFrame.transpose(GDSCE)
# Load GDSC response
GDSCR = pd.read_csv(os.path.join(root_dir,'data/response/')+"GDSC_response.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ",")

PDXE = pd.read_csv(os.path.join(root_dir,'data/expr_homogenized/')+"PDX_exprs.Gemcitabine.eb_with.GDSC_exprs.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ",")
PDXE = pd.DataFrame.transpose(PDXE)

PDXM = pd.read_csv(os.path.join(root_dir,'data/SNA_binary/')+"PDX_mutations.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ".")
PDXM = pd.DataFrame.transpose(PDXM)

PDXC = pd.read_csv(os.path.join(root_dir,'data/CNA/')+"PDX_CNA.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ".")
PDXC = pd.DataFrame.transpose(PDXC)

GDSCM = pd.read_csv(os.path.join(root_dir,'data/SNA_binary/')+"GDSC_mutations.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ".")
GDSCM = pd.DataFrame.transpose(GDSCM)

GDSCC = pd.read_csv(os.path.join(root_dir,'data/CNA/')+"GDSC_CNA.Gemcitabine.tsv", sep = "\t", index_col=0, decimal = ".")
